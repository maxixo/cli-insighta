...
export type TokenManagerErrorCode = "REFRESH_FAILED" | "INVALID_EXPIRY";
...
export type TokenManager = {
  readStoredCredentials(): Promise<StoredCredentials>;
  ...
  withAuthenticatedRequest<TResult>(
    request: (
      accessToken: string,
...
export function createTokenManager(options: {
  ...
  writeCredentials?: typeof writeStoredCredentials;
  deleteCredentials?: typeof deleteStoredCredentials;
}): TokenManager {
  ...
  return {
    ...
    async getValidCredentials(initialCredentials) {
      const credentials =
        initialCredentials ??
    ...
    async refreshStoredSession(initialCredentials) {
      const credentials =
        initialCredentials ??
        (await readCredentials(options.credentialsStoreOptions));
    ...
    async withAuthenticatedRequest(request, requestOptions = {}) {
...
export function shouldRefreshCredentials(
  credentials: StoredCredentials,
  now: () => Date = () => new Date(),
  refreshBufferMs = ACCESS_TOKEN_REFRESH_BUFFER_MS,
): boolean {
